...
CREATE TABLE `allowance_details` (
  `allowance_details_id` int NOT NULL AUTO_INCREMENT,
  `allowance-type` varchar(45) NOT NULL,
  `allowance_amount` double DEFAULT NULL,
  `allowance_id` int NOT NULL,
  `emp_id` int NOT NULL,
  PRIMARY KEY (`allowance_details_id`),
  KEY `FK_emp-id_idx` (`emp_id`),
  KEY `allowance_id` (`allowance_id`),
  CONSTRAINT `allowance_id` FOREIGN KEY (`allowance_id`) REFERENCES `allowance` (`allowance_id`),
  CONSTRAINT `FK_emp-id` FOREIGN KEY (`emp_id`) REFERENCES `employee` (`emp_id`)
